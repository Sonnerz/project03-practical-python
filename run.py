...
def play():
    return render_template("play_v4.html", page_title="Riddle-Me-This - Play", username=session['username'], leaderboard=leaderboard, 
                                    player_info=player_info, usernames=usernames,
                                    riddle_question=riddle['Question'], riddle_answer=riddle['Answer'], riddle_index=riddle['Number'])                                
...
def check_answer(answerInputByPlayer, correct_answer, riddle_index):
    '''
    ...
    for player in player_info:
        if player['username'] == session['username']:
            if correct_answer == answerInputByPlayer:
                ...
                if (riddle_index < len(riddles)): # check for last riddle
                ...
                if player['attempt'] == 2: # max of 2 attempts
                    ...
                    if (riddle_index < len(riddles)): #check for last riddle
                        ...
                        return player['riddle_number'] 
            return riddle_index # index of next riddle
...
def check():
    ...
    if request.method == "POST":
        ...
        riddle_index = check_answer(player_answer, riddle['Answer'], riddles.index(riddle)) # check_answer() called. Index returned.
        if riddle_index > 2: # check for last riddle
            return redirect(url_for('end')) # end of game
        else:
            riddle = get_next_riddle(riddle_index) # not end of game, get next riddle
    return redirect(url_for('play'))    
